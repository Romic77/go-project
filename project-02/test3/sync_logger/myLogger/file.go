...
func (f *FileLogger) writeLogBackground() {
	for {
		...
		//如果从管道中取出来了值就写日志
		select { //select关键字是用来判断，当前管道当前操作是读还是写
		case logTmp := <-f.logChan:
			fmt.Fprintf(f.fileObj, "[%s] [%s] [%s:%s:%d] %s \n", logTmp.theTime, getLogString(f.logLevel), logTmp.fileName, logTmp.funcName, logTmp.lineNo, logTmp.msg)
			if f.logLevel >= ERROR {
				if f.checkSize(f.fileObj) {
					//需要切割日志文件
					newFile, err := f.splitFile(f.fileObj)
...
func (f *FileLogger) log(logLevel LogLevel, msg string) {
	if f.enable(logLevel) {
		...
		logTmp := &logMsg{
			...
			fileName: fileName,
			lineNo:   lineNo,
			theTime:  now.Format("2006-01-02 15:04:05"),
		}
		select {
